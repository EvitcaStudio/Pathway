...
class PathwaySingleton {
	...
	moveInstance(pInstance, pInstanceData, pInstancePosition) {
		...
		if (pInstanceData.mode === 'collision') {
			...
			// This value is for the most part static, but when it changes, it should be an instant change and not happen overtime.
			pInstance.stepPixelsPerSecond = pInstance.moveSettings.stepSize * PathwaySingleton.DEFAULT_BASE_FPS;;
			// Get the speed
