...
b.prototype.copy=b.prototype.clone,b}();q=[];void 0===(B="function"==typeof(l=function(){return m})?l.apply(v,q):l)||(n.exports=B)}).call(this)}]);
Diob.prototype.cancelMove=function(){this.EPathfinderID&&this.easystar&&(this.easystar.cancelPath(this.EPathfinderID),this.easystar.collisionGrid=void 0,this.EPathfinderID=null);this.EPathfinderTrajectory?(this.EPathfinderTrajectory.x=0,this.EPathfinderTrajectory.y=0,this.EPathfinderTrajectory.angle=0,this.EPathfinderTrajectory.lastTime=0,this.EPathfinderTrajectory.deltaTime=0,this.EPathfinderTrajectory.elapsedMS=0,this.EPathfinderTrajectory.nextNodePos=null):this.EPathfinderTrajectory={angle:0,x:0,
y:0,nextNodePos:null,lastTime:0,deltaTime:0,elapsedMS:0};this.EPathfinderPath=[];this.EPathfinderPathReversed=[];this.EPathfinderMoving=!1;this.moveSettings&&(this.moveSettings.stepSlide=!1,this.moveSettings=this.moveSettings);this.EPathfinderOriginalStepSize&&(this.moveSettings.stepSize=this.EPathfinderOriginalStepSize);this.move();clearInterval(this.EPathfinderTrajectory.interval)};
...
"function"===typeof e.onPathComplete)e.onPathComplete(e.EPathfinderID);e.cancelMove()}}else e.EPathfinderTrajectory.angle=EPathfinder.getAngle(g,e.EPathfinderTrajectory.nextNodePos),e.EPathfinderTrajectory.x=Math.cos(e.EPathfinderTrajectory.angle),e.EPathfinderTrajectory.y=Math.sin(e.EPathfinderTrajectory.angle),e.dir=EPathfinder.getDirFromAngle(-e.EPathfinderTrajectory.angle),e.movePos(e.EPathfinderTrajectory.x,e.EPathfinderTrajectory.y),e.EPathfinderMoving=!0;else{var t=e.EPathfinderPath.shift();
t={x:t.x*w.width-w.width/2,y:t.y*w.height-w.height/2};if(EPathfinder.debugging){const m=VYLO.Map.getLocByPos(t.x,t.y,e.mapName),b=VYLO.newDiob("Overlay");b.atlasName="";b.width=w.width;b.height=w.height;b.color={tint:23295};b.alpha=.8;b.plane=0;b.mouseOpacity=0;b.touchOpacity=0;b.setTransition({alpha:0},-1,3E3);m.addOverlay(b);setTimeout(()=>{m.removeOverlay(b)},3E3)}e.EPathfinderTrajectory.angle=EPathfinder.getAngle(g,t);e.EPathfinderTrajectory.nextNodePos=t;e.EPathfinderTrajectory.x=Math.cos(e.EPathfinderTrajectory.angle);
e.EPathfinderTrajectory.y=Math.sin(e.EPathfinderTrajectory.angle);e.dir=EPathfinder.getDirFromAngle(-e.EPathfinderTrajectory.angle);e.movePos(e.EPathfinderTrajectory.x,e.EPathfinderTrajectory.y);e.EPathfinderMoving=!0;if(EPathfinder.debugging){const m=VYLO.newDiob();m.atlasName="";m.color={tint:16777215};m.width=w.width;m.height=5;m.anchor=0;m.plane=0;m.mouseOpacity=0;m.touchOpacity=0;m.density=0;m.angle=-e.EPathfinderTrajectory.angle;m.setPos(g.x,g.y,e.mapName);m.setTransition({alpha:0},-1,3E3);
setTimeout(()=>{VYLO.delDiob(m)},3E3)}}if(g.x===I.x&&g.y===I.y&&(G++,100<=G&&(e.cancelMove(),e.onPathStuck&&"function"===typeof e.onPathStuck))){e.onPathStuck();return}I=g}this.EPathfinderTrajectory.lastTime=Date.now()},16);this.easystar.setAcceptableTiles([0]);this.easystar.disableDiagonals();this.easystar.disableCornerCutting();this.easystar.setGrid([[]]);p&&(this.easystar.enableDiagonals(),this.easystar.enableCornerCutting());p=EPathfinder.mapTilesToGrid(this.mapName,q);A=p.acceptedTiles;var H=
p.weights;this.easystar.setGrid(p.grid);H.forEach(g=>{e.easystar.setTileCost(g,g)});this.easystar.setAcceptableTiles(A);p=Math.round(this.x+this.xOrigin+this.width/2);A=Math.round(this.y+this.yOrigin+this.height/2);n=EPathfinder.clamp(Math.round(EPathfinder.clamp(n-1,0,B.x))*w.width+w.width/2,0,B.xPos-w.width);v=EPathfinder.clamp(Math.round(EPathfinder.clamp(v-1,0,B.y))*w.height+w.height/2,0,B.yPos-w.height);B=VYLO.Map.getLocByPos(p,A,this.mapName);const E=VYLO.Map.getLocByPos(n,v,this.mapName);var k=
EPathfinder.tileToNode(B);H=EPathfinder.tileToNode(E);if(EPathfinder.debugging){const g=VYLO.newDiob("Overlay");g.atlasName="";g.width=w.width;g.height=w.height;g.color={tint:16777215};g.alpha=.7;g.plane=0;g.touchOpacity=0;g.touchOpacity=0;g.setTransition({alpha:0},-1,3E3);E.addOverlay(g);setTimeout(()=>{E.removeOverlay(g)},3E3)}const J=function(g,t,m,b={left:!1,right:!1,up:!1,down:!1},f){if(g){let h={},u={};for(let y=1;7>y;y++){const c=VYLO.Map.getLocByPos(t-y*w.width/2,m,e.mapName),d=VYLO.Map.getLocByPos(t+
y*w.width/2,m,e.mapName),a=VYLO.Map.getLocByPos(t,m-y*w.height/2,e.mapName),r=VYLO.Map.getLocByPos(t,m+y*w.height/2,e.mapName);!c||b.left||c.density&&!q.includes(c)||c.getContents().filter(x=>{const C=f?EPathfinder.within(e.y+e.yOrigin+e.height,x.y+x.yOrigin,x.yOrigin+x.height):!0;if(x.density&&!q.includes(x)&&C)return x.density}).length?(c&&(u[c.id]=c),1>=y&&(b.left=!0)):h[c.id]=c;!d||b.right||d.density&&!q.includes(d)||d.getContents().filter(x=>{const C=f?EPathfinder.within(e.y+e.yOrigin+e.height,
x.y+x.yOrigin,x.yOrigin+x.height):!0;if(x.density&&!q.includes(x)&&C)return x.density}).length?(d&&(u[d.id]=d),1>=y&&(b.right=!0)):h[d.id]=d;!a||b.up||a.density&&!q.includes(a)||a.getContents().filter(x=>{const C=f?EPathfinder.within(e.x+e.xOrigin+e.width,x.x+x.xOrigin,x.xOrigin+x.width):!0;if(x.density&&!q.includes(x)&&C)return x.density}).length?(a&&(u[a.id]=a),1>=y&&(b.up=!0)):h[a.id]=a;!r||b.down||r.density&&!q.includes(r)||r.getContents().filter(x=>{const C=f?EPathfinder.within(e.x+e.xOrigin+
e.width,x.x+x.xOrigin,x.xOrigin+x.width):!0;if(x.density&&!q.includes(x)&&C)return x.density}).length?(r&&(u[r.id]=r),1>=y&&(b.down=!0)):h[r.id]=r}let z;if(EPathfinder.debugging)for(const y in u){const c=VYLO.newDiob("Overlay");c.color={tint:16711680};c.atlasName="";c.width=w.width;c.height=w.height;c.color={tint:f?16753920:16711680};c.alpha=.3;c.plane=0;c.mouseOpacity=0;c.touchOpacity=0;c.setTransition({alpha:0},-1,3E3);u[y].addOverlay(c);setTimeout(()=>{u[y].removeOverlay(c)},3E3)}for(const y in h){if(EPathfinder.debugging){const c=
...
[...t])}else if(!g||!g.length){if(e.onPathNotFound&&"function"===typeof e.onPathNotFound)e.onPathNotFound();e.cancelMove()}e.easystar.collisionGrid=void 0})}console.error("EPathfinder: No mapName was found on this diob.")};
